...
--								 테이블을 구성하는 필드들 중 자격이 되는 필드 중 하나에 부여하는 테이블의 대표 값
-- 기본키를 지정함으로써 기본키가 지정하지 않았을 때 발생할 수 있는 오류들을 미연에 방지할 수 있다.
-- 기본키로 결함을 없애고 운영되는 것을 개체 무결성 이라고 부릅니다.

-- 왜래키(Foregin Key) : 테이블 간의 필수 포함 관계에 있어 상대 테이블의 특정 필드 값을 참조하고
--								 없는 값을 사용하지 않는 필드
-- 외래키로 유지되는 무결성을 참조 무결성이라고 부릅니다.
